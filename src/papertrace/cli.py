...
def _refs_pipeline(
    *,
    ...
    manifest = RefManifest(
        ...
        # what was left out on request. A different gap from a withheld label
        # and never the same one: a `skipped` entry is neither `retrieved` nor
        # `provided`, so it can never become a candidate for withholding.
        limits=limits,
    )
